...
const LeadsTable = () => {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/leads`, {
  ...
  const updateField = async (id, fieldName, value) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/leads/${id}`, {
        method: 'PATCH',
        headers: {
