...
const CmsSettings = () => {
  ...
  const fetchData = async () => {
    try {
      const [settingsRes, packagesRes, testimonialsRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms`),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/packages`),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/testimonials`)
      ]);
  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms`, {
        method: 'POST',
  ...
  const handleAddPackage = async (e) => {
    ...
    try {
      const token = localStorage.getItem('crm_token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/packages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleDeletePackage = async (id) => {
    ...
    try {
      const token = localStorage.getItem('crm_token');
      await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/packages/${id}`, {
        method: 'DELETE',
  ...
  const handleAddTestimonial = async (e) => {
    ...
    try {
      const token = localStorage.getItem('crm_token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/testimonials`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleDeleteTestimonial = async (id) => {
    ...
    try {
      const token = localStorage.getItem('crm_token');
      await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/crm/cms/testimonials/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchData();
